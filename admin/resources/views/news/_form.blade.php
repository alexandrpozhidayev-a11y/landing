...
<div class="grid grid-cols-1 sm:grid-cols-2 gap-6">
    ...
    <div class="space-y-2 sm:pt-6">
        <label class="flex items-center">
            ...
            <span class="ms-2 text-sm text-gray-600">{{ __('Published (visible on the site)') }}</span>
        </label>
        <label class="flex items-center">
            ...
            <span class="ms-2 text-sm text-gray-600">{{ __('Main news (large card on the News page — only one)') }}</span>
        </label>
    </div>
</div>
...
<div>
    ...
    <p class="mt-1 text-xs text-gray-500">{{ __('JPG, PNG or WebP, up to 5 MB.') }}</p>
