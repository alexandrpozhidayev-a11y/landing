...
<div class="grid grid-cols-1 sm:grid-cols-2 gap-6">
    <div>
        <x-input-label for="sort_order" :value="__('Order (smaller — earlier on the site)')" />
    ...
    <div class="space-y-2 sm:pt-6">
        <label class="flex items-center">
            ...
            <span class="ms-2 text-sm text-gray-600">{{ __('Visible on the site') }}</span>
        </label>
        <label class="flex items-center">
            ...
            <span class="ms-2 text-sm text-gray-600">{{ __('Featured (larger card on the Team page)') }}</span>
        </label>
    </div>
</div>

<div>
    <x-input-label for="photo" :value="__('Portrait')" />
    @if ($member?->photo)
    ...
    <p class="mt-1 text-xs text-gray-500">{{ __('JPG, PNG or WebP, up to 5 MB.') }} {{ __('Without a portrait the site shows the "Portrait to follow" placeholder.') }}</p>
